...
<body>
        ...
        <div class="row">
                                    ...
                                    <li class="nav-item">
                                        ...
                                        <div class="collapse show" id="submenu1-1" aria-expanded="false">
                                            <ul class="flex-column pl-4 nav">
                                                ...
                                                <li class="nav-item">
                                                    <a class="nav-link active py-1" href="/predict">
            ...
            <main role="main" class="col-md-10 ml-sm-auto col-lg-10 px-4" id="predMain">
	            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
	                <h1 class="h2">데이터 이상감지</h1>
	                <div class="btn-group">
	                    <button type="button" class="btn btn-primary" id="btnExecutePredict" data-loading-text="실행중...">실행</button>
	                </div>
	            </div>
				...
				<form class="was-validated">
		            ...
		            <div class="d-flex mb-4">
		            	<input type="text" class="form-inline form-control" id="inputDataFilePath" 
		            		placeholder="*.csv, *.txt, *.gz 등   (EX - D:/watchdog/data/test.csv)   ※ 구분자, 띄어쓰기 주의" required/>
		            ...
		            <div class="dropdown mb-4">
		            	...
		            	<div class="alertText mt-1" id="alertTextMetaIsNotExisted"> 해당 프로젝트의 기준정보가 생성되어있지 않습니다. </div>
		            </div>
    ...
    <script type="text/javascript">
		...
		// 이상탐지을 위해 PredictController.java에 넘겨줘야할 정보들
		...
		var checkMeta = function(project) {
			var formData = {
					"projectName" : project
				}
			...
			$.ajax({
				...
				success : function(isExistedMeta) {
					if(isExistedMeta){
						...
						$("#alertTextMetaIsNotExisted").hide();
					}
					else{
						// 2020.04.10 kwb 선택한 프로젝트의 기준정보가 존재하지 않으면, 드롭다운 메뉴의 테마색을 빨간색으로 변경하고 경고문구를 보여줌
						$("#projectNameDropdown").removeClass("btn-outline-success");
						$("#projectNameDropdown").addClass("btn-outline-danger");
						$("#alertTextMetaIsNotExisted").show();

						// 2020.04.10 kwb 기준정보가 생성되지 않은 프로젝트를 선택한 상태로 실행 버튼을 눌렀을 때, 실행을 막기위하여 projectName(전역변수) 초기화
						// 실행 버튼 클릭 이벤트를 처리하는 부분($("#btnExecutePredict").on("click", function() {~})에서 해당 값을 체크하고 있음
						projectName = "";
					}
				},
				error : function(request, status, error) {
		...
		var appendResultTable = function(resultData) {
			...
			var tableHead = "";
	    	tableHead += '<table class="table table-striped table-borderless" id="resultTable"><caption>PSI 이상 기준: 1 / 항목 이상 기준: 변동률 50% </caption><thead><tr><th scope="col">프로젝트명</th><th scope="col">파일명</th><th scope="col">파일 건수</th><th scope="col">PSI</th><th scope="col">이상 항목 건수</th><th scope="col">카이제곱 이상 항목 수</th></tr></thead><tbody id="resultTbody"></tbody></table>';
	    	$('main').append(tableHead);
	    	
			var predRow = "<tr>";
			predRow += '<td>'+resultList.projectName+'</td>';
			predRow += '<td>'+resultList.fileName+'</td>';
			predRow += '<td>'+resultList.fileCount+'</td>';
			predRow += '<td>'+resultList.psi+'</td>';
			predRow += '<td>'+resultList.car+'</td>';
			predRow += '<td>'+resultList.chiSqCnt+'</td>';
			predRow += "</tr>";
			
			$("#resultTbody").empty();
			$("#resultTbody").append(predRow);
	
			$('#resultTable td:nth-child(4)').each(function() {
		...
		$("#btnExecutePredict").on("click", function() {
			// 데이터 파일 경로 값 세팅
			inputDataFilePath = $("#inputDataFilePath").val().replace(/\\/g, "/");
			// 레이아웃 파일 경로 값 세팅 
			inputLayoutFilePath = $("#inputLayoutFilePath").val().replace(/\\/g, "/");
			...
			if(columnCalculateYN.trim().length == 0 || inputDataFilePath.trim().length == 0
				...
				setTimeout(executePredict, 1000);
			}
	    });
